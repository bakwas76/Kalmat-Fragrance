...
const updateStatus = async (id: string, status: OrderStatus) => {
  ...
  try {
    ...
    // Notify the customer by email about the status change (fire-and-forget)
    supabase.functions
      .invoke('update-order-status', {
        body: { order_id: id, new_status: status },
      })
      .then(({ error: fnError }) => {
        if (fnError) {
          console.error('[Admin Orders] Failed to send status email:', fnError.message);
        }
      });
  } catch (error) {
    const message =
          ...
          >
            ...
            >
              ...
              <div className="max-h-[70vh] overflow-y-auto p-6">
                ...
                <div className="mt-6 border-t border-ink-800 pt-6">
                  <p className="text-xs uppercase tracking-wide-sm text-gold">Courier Tracking Link (TCS, Leopard, etc.)</p>
                  <CourierLinkForm order={viewing} onSaved={(url) => {
                    const updated = { ...viewing, courier_tracking_url: url };
                    setViewing(updated);
                    setOrders((prev) => prev.map((o) => (o.id === viewing.id ? updated : o)));
                  }} />
                </div>
...
function CourierLinkForm({ order, onSaved }: { order: Order; onSaved: (url: string) => void }) {
  const { toast } = useToast();
  const [url, setUrl] = useState(order.courier_tracking_url || '');
  const [saving, setSaving] = useState(false);

  const handleSave = async () => {
    const trimmed = url.trim();
    if (!trimmed) {
      toast('Please paste the courier tracking link first', 'error');
      return;
    }
    ...
    try {
      ...
      toast('That does not look like a valid link', 'error');
      return;
    }

    setSaving(true);
    const { error } = await supabase.functions.invoke('send-tracking-link', {
      body: { order_id: order.id, tracking_url: trimmed },
    });
    setSaving(false);

    if (error) {
      toast(`Failed to save/send: ${error.message}`, 'error');
      return;
    }

    toast('Tracking link saved and emailed to customer');
    onSaved(trimmed);
  };
  ...
  return (
    <div className="mt-2 flex flex-col gap-2 sm:flex-row">
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="flex items-center justify-center gap-2 border border-gold/40 bg-gold/10 px-4 py-2 text-xs font-medium text-gold transition-colors hover:bg-gold/20 disabled:opacity-50"
      >
        {saving ? <Loader2 size={13} className="animate-spin" /> : <Package size={13} />}
        {order.courier_tracking_url ? 'Update & Notify' : 'Save & Notify'}
      </button>
    </div>
  );
}
